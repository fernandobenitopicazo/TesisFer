...
En \cite{Mora2012} los autores presentan un novedoso método para calcular el cierre de atributos sobre un conjunto de implicaciones, basado en la lógica \slfd.
...
Para finalizar este apartado, se muestra un ejemplo básico de aplicación del algoritmo {\tt Cls} propuesto.

\begin{ejemplo}
\label{ejemplo:basico} 
	Sean $\Sigma=\{ak\to bc, cd\to gh, cij\to kl, de\to f, g\to de, hf\to ia, f\to c\}$ y $A=\{adf\}$. 
	
	El algoritmo {\tt Cls} devuelve $A^+_\Sigma=\{a,c,d,e,f,g,h,i\}$ y también $\Sigma^\prime = \{k \to b, j \to kl\}$. 
